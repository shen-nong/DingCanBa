...
function addFriends(field){
	  ...
	  if(result.responseText == "1"){
		  $("#addFirends"+field).toggleClass("added_follow");
				...
				<div class="title">
                	...
                	<div class="tiimg pull-left">
                		<a href="user.html?id=${message.user.userId }">
                			<img  alt="${message.user.userName }" src="${message.user.photo50 }" />
               			</a>
                	</div>
                    <div class="titext pull-left">
                    	<div class="titext_1">
                        	<div class="tname pull-left"><a href="user.html?id=${message.user.userId }">${message.user.userName }</a></div>
                            <div class="add_follow pull-left" onclick="addFriends(${message.user.userId })" id="addFirends${message.user.userId }"></div> 
                            <div class="clear"></div>
                        </div>
                        <div class="titext_2">
                        	${message.date  } 买了 
...
<!--^就这么一段玩意儿-->
<style>
	.navigation_container{ width:100%; z-index:100; background:#fff;}
	.navigation_container.a{ position:absolute; top:146px;}
	.navigation_container.b{ position:fixed; top:0;}
</style>
<script>
	var nav_cnt = $('.navigation_container').removeAttr('style');
	$(window).scroll(function () {
		var top
		...
		if (top > 146) {
			nav_cnt.removeClass('a').addClass('b');
		} else {
			nav_cnt.removeClass('b').addClass('a');
		}
	});
	...
	var hahaha = function () {
		hahahaD += 3;
		$('.logo').css({ '-webkit-transform': 'rotate(' + hahahaD + 'deg)' });
		if (hahahaD < 360) setTimeout(hahaha, 5);
	};
	hahaha();
</script>
<!--$就这么一段玩意儿-->
